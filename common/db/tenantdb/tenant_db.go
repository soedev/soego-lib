...
import (
	...
	"github.com/soedev/soego-lib/common/des"
	"github.com/soedev/soego-lib/common/keylock"
	"github.com/soedev/soego-lib/common/utils"
	"log"
	"sync"
	"time"
)
	...
	if enable {
		...
		if tenantDataSourceBack != nil && tenantDataSourceBack.Enable == 1 {
			...
			if tenantDataSource.URL == "" {
				soego_lib.Logger.Error("备用数据源赋值失败")
			}
		} else {
			isNew = false
		}
