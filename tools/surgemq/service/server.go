...
import (
	"errors"
	"fmt"
	"github.com/soedev/soego-lib/tools/surgemq/auth"
	"github.com/soedev/soego-lib/tools/surgemq/message"
	"github.com/soedev/soego-lib/tools/surgemq/sessions"
	"github.com/soedev/soego-lib/tools/surgemq/topics"
	"io"
	"net"
	"net/url"
	"strings"
...
type Server struct {
	...
	//svcs map[uint64]*service  @soeluc edit  注释了无用代码块
...
//获取在线 客户端信息
func (this *Server) GetClients() []Svc {
	var list []Svc
	for _, v := range svcs {
		svc := Svc{ClientID: v.sess.ID(), ConTime: v.time}
		s := v.conn.(net.Conn).RemoteAddr().String()
		svc.IP = s
...
func (this *Server) handleConnection(c io.Closer) (svc *service, err error) {
	...
	svcs[svc.id] = svc
	this.mu.Unlock()
	fmt.Println(fmt.Sprintf("(%s) server/handleConnection: Connection established.", svc.cid()))
	return svc, nil
}
