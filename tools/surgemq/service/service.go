...
var (
	gsvcid uint64              = 0
	svcs   map[uint64]*service = make(map[uint64]*service) //soeluc edit 公共存放连接客户端信息
)
...
func (this *service) stop() {
	...
	if !this.client {
		//从map 中清除连接
		if _, ok := svcs[this.id]; ok {
			delete(svcs, this.id)
			fmt.Println(fmt.Sprintf("(%s) server/handleConnection: connection closed.", this.cid()))
		}
	}
}
